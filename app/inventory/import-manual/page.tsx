...
// --- INLINE EDIT COMPONENT (Extracted to prevent focus loss) ---
...
export default function ManualImportPage() {
  ...
  const handleAddRow = () => {
    const newItem: ManualJewelleryItem = {
      ...
      color: '',
      clarity: '',
      hsn_code: '7113', // Default Jewellery HSN
      remarks: '',
      total_amount: '',
    }
    ...
    try {
      ...
      for (const prefix of Object.keys(groupedByPrefix)) {
        ...
        for (const item of groupedByPrefix[prefix]) {
          ...
          // Calculate aggregate totals automatically
          ...
          inventoryPayload.push({
            ...
            // Auto-Calculated Aggregate Totals
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <Card className="shadow-sm border-slate-200 md:col-span-2">
                <CardContent className="p-5 flex flex-col sm:flex-row gap-4 items-end">
                  ...
                  {/* ADD ROWS */}
                  <div className="flex-1 w-full">
                    <Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">2. Data Entry</Label>
                    <Button 
                      onClick={handleAddRow}
                      variant="outline"
                      className="w-full h-10 border-dashed border-2 border-indigo-200 text-indigo-600 hover:bg-indigo-50 hover:border-indigo-300 font-semibold"
                    >
              ...
              {items.length > 0 ? (
                <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-5 flex flex-col gap-3 justify-center animate-in fade-in">
                ...
                <div className="border border-dashed border-slate-200 rounded-xl flex items-center justify-center text-slate-400 text-sm font-medium">
                  Add rows to begin
                </div>
              )}
            </div>
                  ...
                  <table className="w-full text-left text-sm whitespace-nowrap table-fixed">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {currentItems.map((item) => {
                        ...
                        // Real-time calculation of total diamond weight
                        const solPcs = Number(item.solitaire_pcs) || 0;
                        const solCts = Number(item.solitaire_cts) || 0;
                        const melPcs = Number(item.melee_pcs) || 0;
                        const melCts = Number(item.melee_cts) || 0;
                        ...
                        return (
                          <tr key={item.temp_id} className={`transition-colors ${isSelected ? 'bg-indigo-50/20' : 'hover:bg-slate-50/50'}`}>
                            ...
                            <td className="py-1 px-1 flex flex-col gap-0.5 justify-center mt-0.5 border-r border-slate-100">
                              <EditableCell 
                                value={item.shape} 
                                onChange={(e: any) => handleItemEdit(item.temp_id, 'shape', e.target.value)} 
                                align="center"
                                className="placeholder:text-slate-300 w-full py-0 leading-tight"
                                placeholder="Shape"
                              />

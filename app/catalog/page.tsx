...
import { 
  Search, Package, Loader2, ArrowLeft, ChevronRight, RefreshCw, 
...
export default function CatalogPage() {
  ...
  const fetchCatalog = async () => {
    ...
    try {
      ...
      const processedCatalog = Object.values(grouped).sort((a, b) => a.sku.localeCompare(b.sku));
  ...
  const handleRequestRestock = async (e: React.FormEvent) => {
    e.preventDefault();
    // Resolve which branch is placing the order
    ...
    if (!selectedSku || !appUser || !finalBranchId || finalBranchId === 'ALL') {
      toast({ title: "Validation Error", description: "Please select a valid branch to receive the restock.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Indent Submitted", description: `${quantity}x ${selectedSku.sku} requested from Head Office.` });
      setSelectedSku(null);
      setQuantity("1");
      setRequiredDate("");
      setRemarks("");
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#fafafa]">
      ...
      <main className="p-4 md:p-6 lg:p-8 max-w-[1600px] w-full mx-auto space-y-6 animate-in fade-in duration-500">
        ...
        <div className="bg-white p-3 rounded-xl border border-gray-200 shadow-sm flex flex-col gap-3 transition-all">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
            ...
            {/* HQ BRANCH SELECTOR OR BRANCH LABEL */}
      ...
      {/* RESTOCK MODAL (REDESIGNED TO BLACK/GRAY STANDARD) */}
